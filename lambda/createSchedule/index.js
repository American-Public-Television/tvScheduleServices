...
const getSSMEnv = async () => {
    const params = {
        Name: process.env.SSM_STORE, // SSM parameter name
        WithDecryption: true
    }

    try {
        const response = await ssm.getParameter(params).promise()
        return JSON.parse(response.Parameter.Value) // Parse the JSON content
    } catch (error) {
...
const getPBS_AUTH = async () => {
    ...
    try {
        ...
        console.error('Error fetching /createtv/PBS_AUTH from SSM:', error)
        throw error
    }
}
...
exports.handler = async (event) => {
    ...
    if (requestedPath === '/host-show') {
        try {
            ...
            return {
                statusCode: 200,
                headers: corsHeaders,
                body: JSON.stringify(response.data) // Return raw JSON here
            }
        } catch (error) {
    ...
    if (requestedPath === '/zip-from-ip') {
        ...
        try {
            ...
            // Ensure that the zip is present in the request body
            if (!requestBody.zip) {
                return {
                    statusCode: 400,
                    headers: corsHeaders,
            ...
            // Fetch the data from the PBS API using the zip code
        ...
        try {
            ...
            // Ensure that the callsign and zip are present in the request body
            ...
            // Set up the headers including the PBS auth header
            let headers = {
                'X-PBSAUTH': pbsAuth
            }

            // Fetch the data from the provider URL
            const providerData = await axios.get(url, { headers })
...
const fetchDataForEndpoint = async (requestBody, requestedPath, pbsAuth) => {
    ...
    let headers = {
        'X-PBSAUTH': pbsAuth // Use pbsAuth passed from handler
    }
